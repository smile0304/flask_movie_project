...
from app.models import User,Userlog,Preview,Tag,Movie,Comment,Movicecol
...
#添加电影收藏
@home.route("/movicecol/add/",methods=["GET"])
@user_login_req
def movicecol_add():
    uid = request.args.get("uid","")
    mid = request.args.get("mid","")
    movicecol = Movicecol.query.filter_by(
        user_id = int(uid),
        movie_id = int(mid)
    ).count()
    if movicecol == 1:
...
#电影收藏
@home.route("/movicecol/<int:page>/")
@user_login_req
def movicecol(page=None):
    if page is None:
        page = 1
    ...
    return render_template("home/movicecol.html",page_data=page_data)
...
def search(page=None):
    ...
    movie_count = page_data = Movie.query.filter(
        #模糊匹配
